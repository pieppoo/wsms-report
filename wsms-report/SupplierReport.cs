    ...
    public partial class SupplierReport : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public void PopulateData()
        {
            if (Data != null)
            {
                ...
                lblTotalItem.Text   = Data.SupplierList.Count.ToString();

                if (Data.SupplierList != null && Data.SupplierList.Count > 0)
                {
                    var i = 1;
                    var templateRow = tblDetails.Rows[1];
                    var currRow = templateRow;

                    foreach (var item in Data.SupplierList)
                    {
                        currRow.Cells[0].Text = i.ToString();
                        currRow.Cells[1].Text = item.Name;
                        currRow.Cells[2].Text = item.PhoneNo;
                        currRow.Cells[3].Text = item.Address;
                        currRow.Cells[4].Text = item.Debt;

                        if (!Data.SupplierList.Last().Equals(item))
                        {
